        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
          ...
          {dbExperience.map(exp => (
            <div key={exp.id} className="card" style={{ background: "var(--background)" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", marginBottom: "0.75rem", flexWrap: "wrap" }}>
                <div style={{ width: "5px", height: "5px", borderRadius: "50%", background: "var(--primary)", flexShrink: 0 }}></div>
                <h3 style={{ fontFamily: "var(--app-font-serif)", fontSize: "1.05rem", fontWeight: 400, color: "var(--foreground)", margin: 0 }}>{exp.role}</h3>
                {exp.duration && <span className="highlight-badge">{exp.duration}</span>}
              </div>
